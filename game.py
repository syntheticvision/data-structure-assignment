...
class Board:
    ...
    # part C: Add an undo function to the Board class
    def undo(self):
        if undo_stack:  # Check if there is a previous state to undo
            self.set(undo_stack.pop())
            self.turn -= 1  # Adjust the turn count
            return True
        return False
    ...
    def add_piece(self, row, col, player):
        if self.valid_move(row, col, player):
            undo_stack.append(self.get_board())  # part C: Save the current state
...
while running:
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            running = False
        elif event.type == pygame.KEYDOWN:  #part C: Check for key presses
            if event.key == pygame.K_u:  # part C: Press 'u' to undo
                if not board.undo():
